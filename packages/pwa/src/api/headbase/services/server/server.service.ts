import {
	...
	ServerInfoDto,
	UpdateUserDto,
	UserDto, LoginRequest, VaultDto, UpdateVaultDto,
...
export class ServerService {
	...
	private async getServerUrl(): Promise<string> {
		const currentUser = await this.kvStoreService.get('currentUser', LocalUserDto)
		if (!currentUser) {
	...
	private async query<ResponseType>(options: QueryOptions): Promise<ResponseType> {
		...
		if (!options.noAuthRequired) {
			const accessToken = await this.kvStoreService.get('accessToken', z.string());
	...
	public async login(loginDetails: LocalLoginRequest) {
		...
		await this.kvStoreService.save('currentUser', localUser)
		await this.kvStoreService.save('refreshToken', loginResult.tokens.refreshToken)
		await this.kvStoreService.save('accessToken', loginResult.tokens.accessToken)
	...
	public async logout() {
		...
		const refreshToken = await this.kvStoreService.get('refreshToken', z.string());
		if (!refreshToken) {
		...
		await this.kvStoreService.delete('currentUser')
		await this.kvStoreService.delete('refreshToken')
		await this.kvStoreService.delete('accessToken')
	...
	public async refresh() {
		...
		try {
			...
			await this.kvStoreService.save('accessToken', tokens.accessToken)
			await this.kvStoreService.save('refreshToken', tokens.refreshToken)
		}
		catch(e: unknown) {
	...
	getCurrentUser(): Promise<LocalUserDto|null> {
		return this.kvStoreService.get('currentUser', LocalUserDto)
	}
