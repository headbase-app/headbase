...
export class SyncWebsocketController {
	...
	init(server: Server, config: SyncWebsocketControllerConfig) {
		this.#config = config;
	...
	async handleServerUpgrade(req: IncomingMessage, socket: internal.Duplex, head: Buffer) {
		...
		if (req.url !== this.#config.path || !req.headers.origin || !this.configService.vars.general.allowedOrigins.includes(req.headers.origin)) {
		...
		if (this.#sockets[initialConnection.connectionData.sessionId]) {
			this.purgeSocket(initialConnection.connectionData.sessionId);
		}
		...
		this.wss.handleUpgrade(req, socket, head, async (ws: WebsocketWithConnectionData) => {
			...
			if (initialConnection.rooms) {
				this.changeRooms(initialConnection.connectionData.sessionId, initialConnection.rooms);
			}
	...
	handleConnection(ws: WebsocketWithConnectionData) {
		...
		ws.on("message", async (data) => {
			...
			if (!parsedEvent.success) {
				this.purgeSocket(ws.connectionData.sessionId);
				return;
			}
			...
			switch (event.type) {
				case "ticket": {
					try {
						...
						this.sendEventToSocket(ws, { type: "ack", data: { messageId: event.messageId } });
					} catch (error) {
						...
						this.sendEventToSocket(ws, { type: "error", data: { messageId: event.messageId, identifier: ErrorIdentifiers.SYSTEM_UNEXPECTED } });
					}
					break;
				}
				case "subscribe": {
					try {
						...
						this.changeRooms(ws.connectionData.sessionId, newRooms);
	...
	syncActionCallback(syncAction: SyncAction) {
		if (syncAction.type === "disconnect-session") {
			this.purgeSocket(syncAction.sessionId);
			...
			for (const roomId of syncAction.rooms) {
				this.sendEventToRoom(roomId, syncAction.event, syncAction.ignoreSessions);
			}
		}
	}

	purgeSocket(sessionId: string) {
	...
	sendEventToSocket(ws: WebsocketWithConnectionData, event: ServerSentEvents) {
		// todo: should even sending go via the sessionId and #sockets?
		ws.send(JSON.stringify(event));
	}

	sendEventToRoom(roomId: string, event: ServerSentEvents, ignoreSessions?: string[]) {
	...
	changeRooms(sessionId: string, newRooms: string[]) {
