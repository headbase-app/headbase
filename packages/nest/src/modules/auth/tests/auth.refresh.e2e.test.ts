...
describe("Refresh Auth", () => {
	describe("Success Cases", () => {
		test("When supplying a valid refreshToken, an accessToken and refreshToken should be returned", async () => {
			...
			const { body, statusCode } = await testHelper.client.post("/v1/auth/refresh").send({
				refreshToken,
			});
	...
	describe("Token Revocation", () => {
		test("After a successful refresh, the previous refreshToken should be invalid", async () => {
			...
			const { statusCode } = await testHelper.client.post("/v1/auth/refresh").send({
				refreshToken,
			});
			...
			const { body: retryBody, statusCode: retryStatusCode } = await testHelper.client.post("/v1/auth/refresh").send({
				refreshToken,
			});
	...
	describe("Invalid Data", () => {
		test("When supplying an incorrectly signed refreshToken, the request should fail", async () => {
			...
			const refreshToken = sign({ userId: testUser1.id, type: "refreshToken" }, "aergsethsrjsrj", { expiresIn: "1h" });
