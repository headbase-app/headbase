...
import {useCurrentVault} from "@renderer/services/vaults/vaults.hooks";
...
import {Dialog} from "@renderer/patterns/components/dialog/dialog";
...
export function VaultManagerDialog() {
	...
	useEffect(() => {
		async function handleVaultChange() {
			if (currentVaultQuery.status === SubscriptionResultStatus.SUCCESS) {
		...
		handleVaultChange()
	}, [vaultsService, currentVaultQuery, setOpenTab])
