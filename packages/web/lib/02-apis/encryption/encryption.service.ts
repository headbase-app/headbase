import { z } from "zod";
import { HeadbaseError } from "../../01-common/control-flow.js";
import {ErrorIdentifiers} from "../../01-common/error-identifiers.js";
...
export class EncryptionService {
	...
	static async decryptProtectedEncryptionKey(protectedEncryptionKey: string, password: string): Promise<string> {
		const [_version, base64Metadata, based64WrappedKey] = protectedEncryptionKey.split(":")
		...
		try {
			...
			throw new HeadbaseError({type: ErrorIdentifiers.INVALID_OR_CORRUPTED_DATA, originalError: e})
		}
		...
		try {
			...
			throw new HeadbaseError({type: ErrorIdentifiers.INVALID_PASSWORD_OR_KEY, originalError: e})
		}
