...
export class DeleteVault extends BaseElement {
	...
	render() {
		if (this.vault$.value.status === "loading") {
			return render(html`<p>Loading vault...</p>`, this)
		}
		else if (this.vault$.value.status === "error") {
			return render(html`<p>An error occurred while loading the vault: ${this.vault$.value.errors.join(",")}</p>`, this)
		}
		else if (this.vault$.value.result === null) {
			return render(html`<p>Vault Not Found</p>`, this)
		}
