...
export class DatabasesAPI {
	...
	async unlock(id: string, password: string): Promise<void> {
		...
		this.#eventsService.dispatch( EventTypes.DATABASE_UNLOCK, { id: id })
	}

	async lock(id: string): Promise<void> {
		await KeyStorageService.delete(id)
		this.#eventsService.dispatch( EventTypes.DATABASE_LOCK, { id: id })
	}
	...
	async changePassword(databaseId: string, currentPassword: string, newPassword: string) {
		...
		this.#eventsService.dispatch( EventTypes.DATABASE_CHANGE, { id: databaseId, action: 'change-password' })
	...
	async create(createDatabaseDto: CreateDatabaseDto): Promise<string> {
		...
		this.#eventsService.dispatch( EventTypes.DATABASE_CHANGE, { id: id, action: 'create'})
	...
	async update(id: string, dataUpdate: UpdateDatabaseDto, preventEventDispatch?: boolean): Promise<void> {
		...
		if (!preventEventDispatch) {
			this.#eventsService.dispatch( EventTypes.DATABASE_CHANGE, { id: id, action: 'update' })
		}
	}
	...
	async delete(id: string): Promise<void> {
		...
		this.#eventsService.dispatch( EventTypes.DATABASE_CHANGE, { id: id, action: 'delete' })
	}
