...
export class ServerAPI {
	...
	constructor(config: ServerAPIConfig) {
		...
		this.#eventsService = config.eventsService
	}
	...
	public async login(loginDetails: LocalLoginRequest) {
		...
		this.#eventsService.dispatch('user-login', {
			serverUrl: loginDetails.serverUrl,
			user: localUser
		})
	...
	public async logout() {
		...
		this.#eventsService.dispatch('user-logout', {})
	}
	...
	liveGetCurrentUser() {
		return new Observable<LiveQueryResult<LocalUserDto|null>>((subscriber) => {
			...
			return () => {
				...
				this.#eventsService.unsubscribe(EventTypes.USER_LOGOUT, handleEvent)
			}
		})
	}
}
