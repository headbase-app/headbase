...
export function useContentFormData(options: ContentFormOptions) {
	...
	useEffect(() => {
		...
		if (queryContentTypeId) {
			const contentTypeQuery = headbase.db.contentTypes.liveGet(currentDatabaseId, queryContentTypeId)
	...
	useEffect(() => {
		...
		if (options.contentId) {
			const contentQuery = headbase.db.contentItems.liveGet(currentDatabaseId, options.contentId)
