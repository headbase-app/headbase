"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var sqlite3_mjs_1 = require("../../../lib/sqlite/sqlite3.mjs");
var DatabaseStore = new Map();
function sqliteFactory() {
    var _this = this;
    var sqlite;
    return function () { return __awaiter(_this, void 0, void 0, function () {
        var opfsAvaliable;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (sqlite) {
                        return [2 /*return*/, sqlite];
                    }
                    return [4 /*yield*/, (0, sqlite3_mjs_1.default)()];
                case 1:
                    sqlite = _a.sent();
                    opfsAvaliable = sqlite.capi.sqlite3_vfs_find("opfs");
                    if (!opfsAvaliable) {
                        console.error('[worker] opfs_not_available');
                        self.postMessage({
                            type: 'error',
                            detail: {
                                error: 'opfs_not_available'
                            }
                        });
                        return [2 /*return*/];
                    }
                    return [2 /*return*/, sqlite];
            }
        });
    }); };
}
(function (self) {
    console.debug('[worker] init');
    console.debug('[worker] onmessage register');
    self.onmessage = function (messageEvent) {
        return __awaiter(this, void 0, void 0, function () {
            var sqlite3, db, db, result, db, db, byteArray;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, sqliteFactory()()];
                    case 1:
                        sqlite3 = _a.sent();
                        if (!sqlite3)
                            return [2 /*return*/];
                        if (messageEvent.data.type === 'open') {
                            db = new sqlite3.oo1.OpfsDb({
                                filename: "/headbase-v1/".concat(messageEvent.data.detail.databaseId, ".sqlite3"),
                                vfs: 'multipleciphers-opfs'
                            });
                            try {
                                db.exec({
                                    sql: 'select * from sqlite_master;',
                                    returnValue: 'resultRows'
                                });
                            }
                            catch (e) {
                                console.debug('[worker] error starting');
                                console.error(e);
                                return [2 /*return*/, self.postMessage({
                                        type: messageEvent.data.type,
                                        targetMessageId: messageEvent.data.messageId,
                                        detail: {
                                            success: false,
                                            error: e
                                        }
                                    })];
                            }
                            DatabaseStore.set(messageEvent.data.detail.databaseId, db);
                            return [2 /*return*/, self.postMessage({
                                    type: messageEvent.data.type,
                                    targetMessageId: messageEvent.data.messageId,
                                    detail: {
                                        success: true
                                    }
                                })];
                        }
                        else if (messageEvent.data.type === 'query') {
                            console.debug('[worker] running query');
                            console.debug(messageEvent.data);
                            db = DatabaseStore.get(messageEvent.data.detail.databaseId);
                            if (!db) {
                                return [2 /*return*/, self.postMessage({
                                        type: messageEvent.data.type,
                                        targetMessageId: messageEvent.data.messageId,
                                        detail: {
                                            success: false,
                                            error: 'database_not_opened'
                                        }
                                    })];
                            }
                            result = db.exec({
                                sql: messageEvent.data.detail.sql,
                                bind: messageEvent.data.detail.params,
                                returnValue: 'resultRows'
                            });
                            return [2 /*return*/, self.postMessage({
                                    type: messageEvent.data.type,
                                    targetMessageId: messageEvent.data.messageId,
                                    detail: {
                                        success: true,
                                        result: { rows: result }
                                    }
                                })];
                        }
                        else if (messageEvent.data.type === 'close') {
                            db = DatabaseStore.get(messageEvent.data.detail.databaseId);
                            if (!db) {
                                return [2 /*return*/, self.postMessage({
                                        type: messageEvent.data.type,
                                        targetMessageId: messageEvent.data.messageId,
                                        detail: {
                                            success: false,
                                            error: 'database_not_opened'
                                        }
                                    })];
                            }
                            db.close();
                            DatabaseStore.delete(messageEvent.data.detail.databaseId);
                            return [2 /*return*/, self.postMessage({
                                    type: messageEvent.data.type,
                                    targetMessageId: messageEvent.data.messageId,
                                    detail: {
                                        success: true,
                                    }
                                })];
                        }
                        else if (messageEvent.data.type === 'export') {
                            db = DatabaseStore.get(messageEvent.data.detail.databaseId);
                            if (!db) {
                                return [2 /*return*/, self.postMessage({
                                        type: messageEvent.data.type,
                                        targetMessageId: messageEvent.data.messageId,
                                        detail: {
                                            success: false,
                                            error: 'database_not_opened'
                                        }
                                    })];
                            }
                            byteArray = sqlite3.capi.sqlite3_js_db_export(db);
                            return [2 /*return*/, self.postMessage({
                                    type: messageEvent.data.type,
                                    targetMessageId: messageEvent.data.messageId,
                                    detail: {
                                        success: true,
                                        database: byteArray
                                    }
                                })];
                        }
                        else {
                            console.error(messageEvent);
                            throw new Error("Unrecognised event sent to worker");
                        }
                        return [2 /*return*/];
                }
            });
        });
    };
})(self);
