...
Lit isn't the smoothest or easiest way of building a full application when compared to alternatives like Svelte, Solid, React etc.
The decision to use Lit is a purposeful tradeoff to sacrifice some simplicity and convenience to stay closer to the native web platform and bet on long-term stability.
Having more direct lower level control of the DOM also makes it easier to develop features like custom plugins as there is no need to battle a framework that wants control over the entire lifecycle and rendering of the application.
...
- High barrier of entry for external code contributions as less common, requires more custom architecture and good practises decisions etc.
